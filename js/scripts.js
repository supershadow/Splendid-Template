...
function openSpot(id,url) {
	...
	$("#spots_thumbnails .active").removeClass("active");
	//$(id).parent().parent().addClass('active');
	$('#'+id).addClass('active');
	
	$("#spots_thumbnails .active").removeClass("new");
	...
	$("#overlay").load(url+' #details', function() {
		...
		$("a.closeDetails").click(function(){ 
			$.address.value("");
			//if ($('#spots_thumbnails .active').offset().top > $(window).height())scrollLocation = $('#spots_thumbnails .active').offset().top - 50;
			closeDetails(scrollLocation); 
		});
...
// console.time("2nd-ready");
	...
	$(window).scroll(function() {
		var url = '?direction=next&data[spotsonly]=1&pagenr='+pagenr+$('#getURL').val()+' #spots_thumbnails';

		if($(document).scrollTop() >= $(document).height() - $(window).height() && $(document).height() >= $(window).height() && pagenr > 0 && $("#overlay").is(':hidden')) {
			if(!($("#spots_thumbnails").hasClass("full"))) {
				var scrollLocation = $("div.container").scrollTop();
				$("#overlay").show().addClass('loading');
				
				$("div#overlay").load(url, function() {				
					if($("div#overlay tbody#spots_thumbnails").children().size() < $('#perPage').val()) {
						$("table.footer").remove();
						$("#spots_thumbnails").addClass("full");
					}
					$("#overlay").hide().removeClass('loading');
					
					$("div#spots_thumbnails").append( $($("div#overlay #spots_thumbnails").html()).fadeIn('slow') );
					
					$("div#overlay").empty();
					$("a.cover").mouseover(function(e) { e.preventDefault(); });
...
// console.time("4th-ready");
	...
	var loading = '<img src="'+BaseURL+'templates/splendid/img/loading.gif" height="16" width="16" />';
...
function toggleFilter(id) {
	...
	if($(ul).is(":visible")) {
		...
		ul.prev().children("span.toggle").css({"background": "url('../img/arrow_closed.gif')"});
		ul.prev().children("span.toggle").attr("title", "<t>Expand filter</t>");
	} else {
		ul.show();
		//ul.prev().children("span.toggle").css("background-position", "-77px -98px");
		ul.prev().children("span.toggle").css({"background": "url('../img/arrow_down.gif')"});
...
function retrieveSpots() {
	...
	$("li.info").html("<img src='templates/splendid/img/loading.gif' />");
...
/* ADDITIONAL FUNCTIONS SPLENDID */

function setThumbViewSize() {
	
	var total_content_width = $('#spots_thumbnails').width();
	var min_width_thumb = 180;
	var max_width_thumb = 260;
	
	var max_on_row = Math.round(total_content_width / 180);
	var max_on_row_min_margin = max_on_row * 10;
	
	
	console.log( total_content_width - max_on_row_min_margin );
