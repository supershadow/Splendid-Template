...
//$_GET['image'] = 'http://splendidnas/spotweb/?page=getimage&messageid=41FGzbG8FlQHkXCTwAV65%40spot.net&image%5Bheight%5D=260&image%5Bwidth%5D=260';
...
if ((!$maxWidth && !$maxHeight) || (!$color && $maxWidth >= $width && $maxHeight >= $height))
{
	...
	$lastModifiedString	= gmdate('D, d M Y H:i:s', @filemtime($_GET['image'])) . ' GMT';
	$etag				= md5($data);
	...
	copy($_GET['image'], 'imagecache/'.$_GET['imgid']);
...
$src	= $creationFunction($_GET['image']);
