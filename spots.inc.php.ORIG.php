...
<?php
	...
	foreach($spots as $spot) {
		...
		echo "<td class='category'><a href='" . $spot['caturl'] . "' title=\"" . sprintf(_("Go to category '%s'"), $spot['catshortdesc']) . "\">" . $spot['catshortdesc'] . "</a></td>" .
			 "<td class='title " . $newSpotClass . " ". $tipTipClass . "'>".'<img class="spotinfoimage" src="/spotweb_v2/?page=getimage&amp;messageid='.$spot['messageid'].'&amp;image[height]=100&amp;image[width]=100" alt="Masterchef Australia 2012 S04e18" style="display: block; width: 130px">'."<a data-cats='" . $catData. "'onclick='openSpot(this,\"".$spot['spoturl']."\")' href='".$spot['spoturl']."' title='" . $spot['title'] . "' class='spotlink'>" . $reportSpam . $rating . $markSpot . $spot['title'] . "</a></td>";
		...
		if ($spot['stamp'] > 1290578400 ) {
			...
			if (!empty($spot['sabnzbdurl'])) {
				if ($spot['hasbeendownloaded']) {
					echo "<td class='sabnzbd'><a onclick=\"downloadSabnzbd('".$spot['id']."','".$spot['sabnzbdurl']."')\" class='sab_".$spot['id']." sabnzbd-button succes' title='" . _('Add NZB to SABnzbd queue (you already downloaded this spot) (s)') . "'>".$spot['hasbeendownloaded']."</a></td>";
				} else {
